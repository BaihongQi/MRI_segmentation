...
plt.show()
# add your code here
#find the contours using the function provided
contours= cv2.findContours(labels,cv2.RETR_FLOODFILL,cv2.CHAIN_APPROX_SIMPLE)[1]
temp= []
#create a list to store the retained contours
for contour in contours:
    # travers the contours for each of them check the value
    # if it is between 10 and 50 then we store them in a list
    area = cv2.contourArea(contour)
    if 10<area<50:
        temp.append(contour)
#convert the list to np array for output
temp=np.array(temp)
cv2.drawContours(img_gs,temp,-1,(255,0,0),1)
plt.imshow(img_gs)
